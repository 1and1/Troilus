...
class ListEntityReadQuery<E> extends AbstractQuery<ListRead<EntityList<E>>> implements ListRead<EntityList<E>> {
    ...
    @Override
    protected ListRead<EntityList<E>> newQuery(Context newContext) {
        return queryFactory.newListSelection(newContext, read, clazz);
    }

    @Override
    public ListRead<EntityList<E>> withDistinct() {
        return queryFactory.newListSelection(getContext(), read.withDistinct(), clazz);
    }
    
    @Override
    public ListRead<EntityList<E>> withFetchSize(int fetchSize) {
        return queryFactory.newListSelection(getContext(), read.withFetchSize(fetchSize), clazz);
    }
    
    @Override
    public ListRead<EntityList<E>> withAllowFiltering() {
        return queryFactory.newListSelection(getContext(), read.withAllowFiltering(), clazz);
    }
    
    @Override
    public ListRead<EntityList<E>> withLimit(int limit) {
        return queryFactory.newListSelection(getContext(), read.withLimit(limit), clazz);
    }
