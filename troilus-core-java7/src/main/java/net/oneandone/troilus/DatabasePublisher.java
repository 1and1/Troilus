...
class DatabasePublisher implements Publisher<Record> {
    
    private boolean subscribed = false; // true after first subscribe
    
    private Datasource datasource = null;
    private DatabaseSubscription pendingSubscription = null;;

    
    public DatabasePublisher(final ListenableFuture<ResultList<Record>> recordsFuture) {
    ...
    private void init(ListenableFuture<ResultList<Record>> recordsFuture) {
        synchronized (this) {
            this.datasource = new Datasource(recordsFuture);
    ...
    public void subscribe(Subscriber<? super Record> subscriber) {
        synchronized (this) {
            if (subscribed == true) {
                ...
                DatabaseSubscription databaseSubscription = new DatabaseSubscription(subscriber);
                if (datasource == null) {
    ...
    private static class DatabaseSubscription implements Subscription {
        ...
        public DatabaseSubscription ready(Datasource datasource) {
