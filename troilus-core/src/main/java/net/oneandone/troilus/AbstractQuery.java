...
abstract class AbstractQuery<Q> {
     ...
     */
    AbstractQuery(final Context ctx) {
        this.ctx = ctx;
    }
     ...
     */
    abstract protected Q newQuery(final Context newContext);
     ...
     */
    public Q withConsistency(final ConsistencyLevel consistencyLevel) {
     ...
     * @return a cloned query instance with the modified behavior
     */
    public Q withRetryPolicy(final RetryPolicy policy) {
     ...
     * @return a cloned query instance with the modified behavior
     */
    public Q withWritetime(final long writetimeMicrosSinceEpoch) {
     ...
     * @return a cloned query instance with the modified behavior
     */
    public Q withTtl(final Duration ttlSec) {
     ...
     * @return a cloned query instance with the modified behavior
    */
    public Q withSerialConsistency(final ConsistencyLevel consistencyLevel) {
     ...
     */
    protected CompletableFuture<ResultSet> performAsync(final DBSession dbSession, final CompletableFuture<Statement> statementFuture) {
        // use async executor to avoid handling with database I/O thread, which could lead to blocking behavior
        return statementFuture.thenComposeAsync(statement -> performAsync(dbSession, statement));
    }
     ...
     * @return the result future 
     */
    protected CompletableFuture<ResultSet> performAsync(final DBSession dbSession, final Statement statement) {
